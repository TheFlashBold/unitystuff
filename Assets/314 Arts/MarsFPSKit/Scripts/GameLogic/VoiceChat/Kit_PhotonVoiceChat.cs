...
namespace MarsFPSKit
{
    ...
    public class Kit_PhotonVoiceChat : Kit_VoiceChatBase
    {
        ...
        public override void JoinedTeam(int team)
        {
            ...
            //Set audio groups
            PhotonVoiceNetwork.Client.ChangeAudioGroups(toLeave, toJoin);
#endif
        }
#if !UNITY_WEBGL

        public void DeviceChanged(string device)
        {
            if (currentBehaviour)
            {
                currentBehaviour.recorder.MicrophoneDevice = device;
            }
        }
        ...
        void Update()
        {
            if (currentBehaviour)
            {
                ...
                if (Kit_GameSettings.voiceChatTransmitMode == VoiceTransmitMode.PushToTalk)
                {
                    ...
                    currentBehaviour.recorder.Detect = false;
                    if (Input.GetButton("Push To Talk"))
                    {
                        currentBehaviour.recorder.Transmit = true;
                    }
                    else
                    {
                        currentBehaviour.recorder.Transmit = false;
                    }
                }
                else if (Kit_GameSettings.voiceChatTransmitMode == VoiceTransmitMode.Detection)
                {
                    //Enable detector
                    currentBehaviour.recorder.Detect = true;
                }
                    ...
                    if (main.currentGameModeBehaviour.isTeamGameMode)
                    {
                        if (main.assignedTeamID != 2)
                        {
                            if (Input.GetButtonDown("Voice Chat Global"))
                            {
                                ...
                                if (PhotonVoiceNetwork.Client.GlobalAudioGroup != 0)
                                {
                                    //Set global
                                    PhotonVoiceNetwork.Client.GlobalAudioGroup = 0;
                                    //Inform player
                            ...
                            else if (Input.GetButtonDown("Voice Chat Team"))
                            {
                                ...
                                if (PhotonVoiceNetwork.Client.GlobalAudioGroup != main.assignedTeamID + 1)
                                {
                                    PhotonVoiceNetwork.Client.GlobalAudioGroup = (byte)(main.assignedTeamID + 1);
